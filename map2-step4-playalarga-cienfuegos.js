// map2-step4-playalarga-cienfuegos.js
// CUBA2 with calibrated positions - STEP 4: Adding Playa Larga → Cienfuegos route
// Now drawing FOUR routes: Habana → Viñales → Soroa → Playa Larga → Cienfuegos

import { latLngToSVG, getCityYAdjustment, getCityLabelPosition } from './projection-config.js';

window.addEventListener('DOMContentLoaded', () => {
  console.log('map2-step4-playalarga-cienfuegos.js loaded (CUBA2 - Step 4: + Playa Larga → Cienfuegos)');
  ...
  setTimeout(() => {
    ...
    const exportSVG = (svgDoc) => {
      ...
      link.href = url;
      link.download = 'cuba-map2-step4-playalarga-cienfuegos.svg';
      link.click();
    ...
    const initializeMap = () => {
      console.log('initializeMap called (CUBA2 Step 4: ... → Playa Larga → Cienfuegos)');
      ...
      // Load GeoJSON data and add markers + FOUR routes
      fetch('cuba-locations-geojson.json')
        ...
        .then((data) => {
          ...
          // STEP 4: Draw Playa Larga → Cienfuegos route
